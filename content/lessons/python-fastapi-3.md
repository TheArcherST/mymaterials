...
Можно было бы хранить в глобальной переменной CHATS сразу экземпляры класса Message. Тогда не надо было бы приводить Message к словарю, а потом обратно.  Однако нам понадобится делать подобные конвертации когда мы начнём использовать базы данных: там нельзя будет сохранить целый экземпляр класса Pydantic модели.
