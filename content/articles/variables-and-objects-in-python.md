...
Представим, что перед нами лист бумаги, на котором написано всё то, что компьтер сохраняет в оперативную память.  Как уже было сказано, там хранится то, с чем компьютер работает в данный момент.  Ну, кажется логичным, что там должна быть записана строка с текстом `Hello World`.  Это действительно так, компьютер загрузит в оперативную память эту строку.  Давайте отразим это на бумаге:
