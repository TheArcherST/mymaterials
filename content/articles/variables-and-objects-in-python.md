...
Как и было нужно, мы в памяти записали тип объекта и значение объекта.  Готово, объект создан.  Но как им пользоваться?
Чтобы использовать этот объект, нам нужно как-то указать на него, и указать, что мы хотим с ним сделать.

Мы могли бы указать пальцем на нарисованный прямоугольник, но Python такого не поймёт. В Python мы можем указывать на объекты только с помощью ссылок.  Ссылка — это данные, по которым Python находит место в памяти (находит наш прямоугольник).
...
Затем получает ссылку на него, и подписывает эту ссылку именем `a` (или, как ещё говорят, связывает имя `a` со ссылкой).
...
В целом, эти два механизма будут нас сопровождать при разработке на Python постоянно, потому важно уметь бегло считывать использование этих механизмов при чтении кода: видеть их и понимать, что именно они делают.
